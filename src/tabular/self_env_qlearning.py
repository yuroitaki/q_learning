...
def qLearning(env,num_episode,gamma,learning_rate,game_step,punish,discount_noise,punish_val,diminishing_weight,max_epsilon,min_epsilon):
    ...
    Q = np.zeros([env._obs_space_n,env._agent._action_space_n])
    goals = []
    counter  = 0
    action_count = []
    ...
    for episode in range(num_episode):
        ...
        while count <= game_step:
            ...
            if discount_noise == True and diminishing_weight == True:
                action = np.argmax(Q[state,:]+np.random.randn(1,env._agent._action_space_n)*(1/(episode+1)))
            elif discount_noise == True and diminishing_weight == False:
                action = np.argmax(Q[state,:]+np.random.randn(1,env._agent._action_space_n))
            else:
...
def main():
    game = "windy_maze" #"FrozenLakeNotSlippery-v0"
    env = map_env.makeMapEnv(game)
