...
def main():
    ...
    for i in range(run_cnt):
        rewards = randomSampling(maze,num_epi,game_cnt)
    ...
    min_epsilon = 0.001
    discount_noise = True                # False to use epsilon-greedy
    ...
    for i in range(run):
        ...
        ############## Store the Result ###############
        '''
        filename = "Tabular_QLearning_Result_of_{0}_{1}_episodes".format(folder,max_episode) 
        ...
        '''
        title = "{0}th_Tabular_QLearning_Result_of_{1}_{2}_episodes".format(i,folder,max_episode)
        # goals = [1,2,3,4,5,6]
