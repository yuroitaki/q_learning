...
class Tabular_Q_Agent:
    ...
    def act(self,state,episode,rate,constant=0.0):
        ...
        if self.discount_noise == True and self.diminishing_weight == True:
            ...
            action = self.epsilonGreedy(state,episode,rate,constant)
    ...
    def risk_train(self,new_state,reward,state,action,risk_level,epi,rate,learning_rate,const=0.0):
        
        # new_action = self.optimalAction(self.U,new_state)
        new_action = self.epsilonGreedy(new_state,epi,rate,const)
    ...
    def epsilonGreedy(self,state,episode,rate,constant=0.0):
